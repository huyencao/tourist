...
<div class="col-xs-12 col-sm-9 content">
    <div class="panel panel-default">
        ...
        <form action="{{ route('catenews.update', $categorynews->id) }}" method="POST">
            {{ csrf_field() }}
            {{ method_field('PUT') }}
                            ...
                            <div class="row">
                                <div class=" col-md-12">
                                    <div class="form-group">
                                        <label class="col-md-1 cotitlentrol-label">{{ __('label.cate_tour.title') }}
                                        </label>
                                        <div class="col-md-6">
                                            <input type="text" placeholder="{{ __('label.enter_title') }}" id="name"
                                                class="form-control" name="name" value="{{ $categorynews->name }}">
                                        </div>
                                    </div>
                                </div>
                                <div class=" col-md-12">
                                    <div class="form-group">
                                        <label
                                            class="col-md-1 control-label">{{ __('label.cate_tour.cate_parent') }}</label>
                                        <div class="col-md-3">
                                            <select name="parent_id" class="selecter_1">
                                                <option value="0"> {{ __('label.cate_tour.create.choose_parent') }}
                                                </option>
                                                @foreach ($parent_cate as $news)
                                                <option value="{{ $news->id }}" @if ($categorynews->parent_id ==
                                                    $news->id) selected @endif>{{ $news->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
